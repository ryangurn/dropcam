...
User.prototype.getSessionToken = function(callback) {
	http.get({ url: ENDPOINT.API_GET_SESSION_TOKEN, headers: { 'Cookie': this.session.cookies.join(',') } }, 
		function(err, data) {
			if(err)
				...
				return callback(new Error(data['status_detail'] || 'Failed to retrieve session token'), null);
		}
	);
};
