...
Camera.prototype.listen = function() {
	var exec = function(done) {
		var cookies = this.user.session.cookies.join(',');
		http.get({ nexus: true, url: util.format('get_cuepoint?uuid=%s&start_time=1415892379&human=false', this.properties['uuid']), 
...
Camera.prototype.capture = function(callback, width) {
	http.get({ url: util.format('%s?uuid=%s&width=%s', ENDPOINT.API_CAMERA_IMAGE, this.properties['uuid'], (width || 600)), 
		headers: { 'Cookie' : this.user.session.cookies.join(',') } }, 
		function(err, data) {
...
Camera.prototype.record = function(seconds, callback) {
	...
	//return new Stream(this.properties['uuid'], this.properties['live_stream_host'], this.user.session.token).start(seconds, callback); 
