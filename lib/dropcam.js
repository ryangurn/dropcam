module.exports = (function() {
	'use strict';

	// General endpoints
	var ENDPOINT = {
		API_LOGIN: 'login.login',
		API_VISIBLE_CAMERAS: 'cameras.get_visible',
		API_CAMERA_BY_PUBLIC_TOKEN: 'cameras.get_by_public_token',
		API_DROPCAM_DEMOS: 'cameras.get_demo'
	}

	var async = require('async');

	var User = require('./dropcam/user'),
		Session = require('./dropcam/session'),
		Camera = require('./dropcam/camera'),
		http = require('./dropcam/http');

	// Short hand function to build an array of cameras asynchronously
	function createCameras(user, items, callback) {
		var cameras = [];

		async.each(items, 
			function(item, _callback) {
				Camera.Create(user, item, function(err, camera) {
					if(err) throw err;
					cameras.push(camera);
					_callback(null, cameras);
				}); 
			}, function(err, results) {
				if(err) callback(err, null);
				callback(null, cameras);
			}
		);
	}

	return {
		// Performs a login to the API
		// Returns a user object containing a valid session if successful
		login: function(username, password, callback) { // POST requests for this endpoint doesn't seem to work. Should not be using GET...
			http.get({ url: ENDPOINT.API_LOGIN + '?username=' + username + '&password=' + password }, 
				function(err, data, cookies) {
					if(err) {
						callback(err, null);
					}
					else if(data['status'] === 0 && data['status_description'] === 'ok') {
						var session = new Session(cookies, data.items[0].session_token);
						User.Create(session, function(err, user) {
							if(err) 
								callback(err, null);
							else
								callback(null, user);
						});
					}
					else {
						callback(new Error(data['status_detail'] || 'Failed to login'), null);
					}
				}
			);
		},
		// Searches this API for camera by public token id
		search: function(user, public_token, callback) {
			http.get({ url: ENDPOINT.API_CAMERA_BY_PUBLIC_TOKEN + '?token=' + public_token,
				headers: { 'Cookie' : user.session.cookies.join(',') } }, 
				function(err, data) {
					if(err) {
						callback(err, null);
					}
					else if(data['status'] === 0 && data['status_description'] === 'ok') {
						Camera.Create(user, data.items[0], function(err, camera) {
							if(err) 
								callback(err, null)
							else 
								callback(null, camera);
						});
					}
					else {
						callback(new Error(data['status_detail'] || 'Failed to search'), null);
					}
				}
			)
		},
		// Returns demos available from the dropcam source
		getDemos: function(user, callback) {
			http.get({ url: ENDPOINT.API_DROPCAM_DEMOS, 
				headers: { 'Cookie' : user.session.cookies.join(',') } },
				function(err, data) {
					if(err) {
						callback(err, null);
					}
					else if(data['status'] === 0 && data['status_description'] === 'ok') {
						createCameras(user, data.items, callback);
					}
					else {
						callback(new Error(data['status_detail'] || 'Failed to get demos'), null);
					}
				}
			);
		},
		// Returns all visible cameras associated to the user
		getVisibleCameras: function(user, callback) {
			http.get({ url: ENDPOINT.API_VISIBLE_CAMERAS, headers: { 'Cookie' : user.session.cookies.join(',') } }, 
				function(err, data) {
					if(err) {
						callback(err, null);
					}
					else if(data['status'] === 0 && data['status_description'] === 'ok') {
						createCameras(user, data.items, callback);
					}
					else {
						callback(new Error(data['status_detail'] || 'Unable to fetch cameras'), null);
					}
				}
			);
		}
	};

})();